...
app.post("/users", function(req, res) {
  ...
  if (!user || !user.name) {
    console.error("ERROR: can't create user (missing user name)");
  ...
  if (existing) {
    console.error(`ERROR: user already exists: ${existing.name}`);
    return res.status(403).json({
      success: false,
...
app.put("/users/:name", function(req, res) {
  ...
  if (existing) {
    console.error(`ERROR: can't rename user (user ${newName} already exists)`);
    return res.status(403).json({
      success: false,
